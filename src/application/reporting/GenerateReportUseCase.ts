 ...
 * Coordinates audit report generation, data preparation, and printing/export formatting.
 */
import {
  Audit,
  Patient,
...
export class GenerateReportUseCase {
  renderHTML(data: AuditReportData): string {
    logger.info('GenerateReportUseCase', `Generando HTML de informe para auditoría ${data.audit.auditCode}`);
    const {
      audit,
      patient,
      ips,
...
<head>
  ...
  <style>
    ...
    .field-value { font-size: 11px; font-weight: 600; color: #0f172a; }
    table { width: 100%; border-collapse: collapse; font-size: 11px; margin-top: 6px; }
    th { background: #f1f5f9; color: #334155; text-align: left; padding: 6px 8px; border: 1px solid #cbd5e1; font-weight: 700; }
    ...
    .signature-area { margin-top: 30px; display: grid; grid-template-columns: 1fr 1fr; gap: 30px; page-break-inside: avoid; }
    .signature-line { border-top: 1px solid #0f172a; padding-top: 6px; font-weight: 600; font-size: 11px; }
...
<body>
  ...
  printPDF(reportData: AuditReportData, _options?: PDFExportOptions): void {
  ...
  exportCSV(audits: Audit[], patients: Patient[], ipsList: IPS[]): string {
